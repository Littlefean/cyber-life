...
class MainWindow(QWidget):
    ...
    def paintEvent(self, event):
        ...
        pixmap = QPixmap(f"assets/fish_0.png")
        ...
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().rotate(45))
        painter.drawPixmap(100, 80, pixmap)  # 顺时针旋转45°
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().rotate(45).scale(-1, 1))
        painter.drawPixmap(180, 80, pixmap)  # 顺时针旋转45°坐标轴，scale在新坐标轴上放缩，即在（2，2）的基础上x轴反向
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().scale(-1, 1).rotate(45))
        painter.drawPixmap(260, 80, pixmap)  # scale使x轴反向，所以rotate旋转方向变了，后面探讨变的规则是啥
        
		# 第三行
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().shear(2, 0))
        painter.drawPixmap(20, 140, pixmap)  # y轴坐标不变（因为第二个参数是0），x轴坐标为y轴坐标乘以第一个参数+x轴原本坐标
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().rotate(45).scale(0.5, 1))
        painter.drawPixmap(100, 140, pixmap)  # 坐标轴旋转后放缩，与（3，3）对比
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().scale(0.5, 1).rotate(45))
        painter.drawPixmap(180, 140, pixmap)  # 旋转和放缩先后顺序不同，结果可能不同
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().scale(-1, 1))
        painter.drawPixmap(260, 140, pixmap)  # scale第一个参数是x轴方向放缩，第二个参数是y轴方向放缩
        
		# 第四行
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().shear(2, 0).scale(0.5, 1))
        painter.drawPixmap(20, 200, pixmap)  # 与（3，1）对比，发现扭曲会改变坐标轴方向
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().scale(0.5, 1).shear(2, 0))
        painter.drawPixmap(100, 200, pixmap)  # 与（4，1）对比，扭曲和放缩顺序不同，结果可能不同
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().scale(0.5, 1))
        ...
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().shear(-sin(pi/4), sin(pi/4)).scale(cos(pi/4), cos(pi/4)))
        painter.drawPixmap(20, 260, pixmap)
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().scale(cos(pi/4), cos(pi/4)).shear(-sin(pi/4), sin(pi/4)))
        painter.drawPixmap(100, 260, pixmap)
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().shear(-sin(pi/3), sin(pi/3)).scale(cos(pi/3), cos(pi/3)))
        painter.drawPixmap(180, 260, pixmap)
        pixmap = QPixmap(f"assets/fish_0.png").transformed(QTransform().scale(cos(pi/3), cos(pi/3)).shear(-sin(pi/3), sin(pi/3)))
